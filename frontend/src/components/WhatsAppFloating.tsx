...
export default function WhatsAppFloating() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[999]">
      {/* Tooltip */}
      {showTooltip && (
        <div className="absolute bottom-20 right-0 bg-gray-900 text-white px-4 py-2 rounded-lg text-sm font-semibold whitespace-nowrap shadow-xl">
      ...
      {/* WhatsApp Button */}
      <a
        href="https://wa.me/917017714708?text=Hi!%20I%20need%20help%20planning%20my%20trip."
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Chat on WhatsApp"
        onMouseEnter={() => setShowTooltip(true)}
        onMouseLeave={() => setShowTooltip(false)}
        className="relative w-16 h-16 flex items-center justify-center rounded-full bg-gradient-to-br from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 shadow-2xl transition-all duration-300 hover:scale-110 group"
      >
        {/* WhatsApp Icon */}
        <svg
          viewBox="0 0 32 32"
        ...
        <span className="absolute -top-1 -right-1 flex h-5 w-5">
          ...
          <span className="relative inline-flex rounded-full h-5 w-5 bg-red-500 items-center justify-center text-white text-xs font-bold border-2 border-white">
            1
          </span>
        </span>
      </a>
